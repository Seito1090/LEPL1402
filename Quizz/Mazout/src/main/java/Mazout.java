...
public class Mazout {
    ...
    public static int getTotalDemand(City city, House firstHouse, String[] path) {
        //TODO make this work :)
        //It's never said we have to visit all the houses in the city, just the ones in the path so
        //unlucky for those that are not included I guess but I don't have to check if the path
        //covers the whole city so that's nice
        int totalDemand = 0;
        int[] visited = new int[city.getNumberOfHouses()];
        // we know that the first house is always valid (SAID IN TEAMS CHAT) but
        // still check for it being null (just in case heh) and we can update totalDemand,
        // visited and currentHouse accordingly
        if (firstHouse == null) {
            throw new IllegalArgumentException();
        }
        House currentHouse = firstHouse;
        totalDemand += currentHouse.getCapacity();
        visited[currentHouse.getIndex()] = 1;
        //now we can iterate through the path and 1) check if all the path is valid (i.e. no nulls)
        //2) update totalDemand, currentHouse and visited accordingly
        for (String direction : path) {
            if (direction == "right") {
                if (currentHouse.getNeighborRight() != null) {
                    currentHouse = currentHouse.getNeighborRight();
                    if (visited[currentHouse.getIndex()] == 0) {
                        totalDemand += currentHouse.getCapacity();
                        visited[currentHouse.getIndex()] = 1;
                    }
                } else {
                ...
                if (currentHouse.getNeighborLeft() != null) {
                    currentHouse = currentHouse.getNeighborLeft();
                    if (visited[currentHouse.getIndex()] == 0) {
                        totalDemand += currentHouse.getCapacity();
                        visited[currentHouse.getIndex()] = 1;
                    }
                ...
                if (currentHouse.getNeighborAbove() != null) {
                    currentHouse = currentHouse.getNeighborAbove();
                    if (visited[currentHouse.getIndex()] == 0) {
                        totalDemand += currentHouse.getCapacity();
                        visited[currentHouse.getIndex()] = 1;
                    }
                ...
                if (currentHouse.getNeighborDown() != null) {
                    currentHouse = currentHouse.getNeighborDown();
                    if (visited[currentHouse.getIndex()] == 0) {
                        totalDemand += currentHouse.getCapacity();
                        visited[currentHouse.getIndex()] = 1;
                    }
        ...
        //then just return the totalDemand
        return totalDemand;
